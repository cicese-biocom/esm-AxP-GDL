...
def predict(ALNDIR, NPZDIR, MDIR):
    ...
    w, b, beta_, gamma_ = load_weights(MDIR)
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            for filename in os.listdir(ALNDIR):
                if not filename.endswith(".a3m"):
                    continue

                # parse & predict
                a3m = parse_a3m(ALNDIR + '/' + filename)
                ...
                npz_file = NPZDIR + filename[:-3] + 'npz'
